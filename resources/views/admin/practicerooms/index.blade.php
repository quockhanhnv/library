...
<div class="m-section">
	<div class="form-group m-form__group row align-items-center">
		...
		<div class="col-md-3" style="padding-left: 100px">
			<a href="{{route('practiceroom.add')}}" class="btn btn-info m-btn m-btn--custom m-btn--icon m-btn--air">
					<span>
	...
	<table class="table table-bordered table-hover">
		...
		<tbody>
			@foreach($practicerooms as $element)
			<tr>
				<td>{{$loop->index + 1}}</td>
				<td>{{$element->practicerooms_name}}</td>
				<td>{{$element->practicerooms_computer_numbers}}</td>
				<td>
					@if($element->practicerooms_active)
				...
				<td data-field="" class="m-datatable__cell">
					<span class="action" style="overflow: visible; position: relative; width: 120px;">							
						<a href="{{route('practiceroom.edit', ['id'=> $element->id])}}" class="m-portlet__nav-link btn m-btn m-btn--hover-info m-btn--icon m-btn--icon-only m-btn--pill" title="Cập nhật">								
							<i class="la la-edit"></i>							
						</a>							
						<a href="javascript:;" onclick="comfirmRemove('{{route('practiceroom.remove',['id' => $element->id])}}')" data-value="42" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill m-btn--delete" title="Xóa">								
